...
axon_type = menu('Choose desired axon:','Original axon','No axon','Eyal axon','Myelinated stick axon', 'Myelinated original axon');
...
if(length(trees) == 1)
    tree{1,1} = trees;
    %tree{1,1} = resample_tree(trees, resampling_interval);
    neuron.params.exchfolder = strcat('../Model/',name);
else
    %tree{1,1} = trees{cell_num};
    tree{1,1} = resample_tree(trees{cell_num}, resampling_interval);
