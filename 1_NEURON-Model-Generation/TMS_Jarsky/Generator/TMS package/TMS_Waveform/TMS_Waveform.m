...
delay_start = 20; % delay at the beginning before TMS delivery
delay_end = 20; % delay after the TMS delivery
ipi = round(ipi/dt)*dt;
...
%xlim([min(train_t) max(train_t)]*1.1);
%ylim([min(train_E) max(train_E)]*1.1);
%% Save train
